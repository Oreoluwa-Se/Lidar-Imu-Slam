...
//     std::deque<CalibState> sig_extended(signal_in.size() + 2 * butter.extend_num);
//     auto it_extended = sig_extended.begin();
//     for (auto it = signal_in.begin(); it != it_front; it++)
//     {
//         *it_extended++ = *it_front;
//     }
//     for (auto it = signal_in.begin(); it != signal_in.end(); it++)
//     {
//         *it_extended++ = *it;
//     }
//     for (auto it = it_back + 1; it != signal_in.end(); it++)
//     {
//         *it_extended++ = *it_back;
//     }

//     signal_out.resize(signal_in.size());
//     tbb::parallel_for(
//         tbb::blocked_range<int>(butter.Coeff_size - 1, sig_extended.size() - butter.Coeff_size),
...
//                 {
//                     auto it_sig_out = signal_out.begin() + i - jj;
//                     temp_state -= *it_sig_out * butter.Coeff_a[jj];
//                 }
//                 signal_out[i - butter.Coeff_size + 1] = temp_state;
//             }
//         });
// }
...
//         move_start_time - init_time_allowance, [](const CalibState &state, double t)
...
//     std::deque<CalibState> IMU_states_all_origin(IMU_state_group_ALL.begin(), IMU_state_group_ALL.end() - 1);
...
//     Eigen::MatrixXd Jacobian(Jaco_size, 9);
//     Jacobian.setZero();
...
//         Jacobian.block<3, 3>(3 * i, 3) << utils::skew_matrix(STD_GRAV);
...
// void SensorInit::print_initialization_result(double &time_L_I, M3D &R_L_I, V3D &p_L_I, V3D &bias_g, V3D &bias_a, V3D grav)
// {
